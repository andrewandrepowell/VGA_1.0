proc create_ipi_design { offsetfile design_name } {
	...
	connect_bd_intf_net [get_bd_intf_pins master_0/M_AXI_LITE] [get_bd_intf_pins VGA_0/s_axi_config]

	# Create port connections
	connect_bd_net -net aclk_net [get_bd_ports ACLK] [get_bd_pins master_0/M_AXI_LITE_ACLK] [get_bd_pins VGA_0/s_axi_config_ACLK]
	connect_bd_net -net aresetn_net [get_bd_ports ARESETN] [get_bd_pins master_0/M_AXI_LITE_ARESETN] [get_bd_pins VGA_0/s_axi_config_ARESETN]

	# Create External ports
	set m_axi_fb_INIT_AXI_TXN [ create_bd_port -dir I m_axi_fb_INIT_AXI_TXN ]
	set m_axi_fb_ERROR [ create_bd_port -dir O m_axi_fb_ERROR ]
	set m_axi_fb_TXN_DONE [ create_bd_port -dir O m_axi_fb_TXN_DONE ]
...
connect_bd_intf_net [get_bd_intf_pins slave_0/S_AXI] [get_bd_intf_pins VGA_0/m_axi_fb]
	# Create port connections
	connect_bd_net -net aclk_net [get_bd_ports ACLK] [get_bd_pins slave_0/S_AXI_ACLK] [get_bd_pins VGA_0/m_axi_fb_ACLK]
	connect_bd_net -net aresetn_net [get_bd_ports ARESETN] [get_bd_pins slave_0/S_AXI_ARESETN] [get_bd_pins VGA_0/m_axi_fb_ARESETN]
	connect_bd_net -net init_axi_txn_00 [get_bd_ports m_axi_fb_INIT_AXI_TXN] [get_bd_pins VGA_0/m_axi_fb_INIT_AXI_TXN]
	connect_bd_net -net error_00 [get_bd_ports m_axi_fb_ERROR] [get_bd_pins VGA_0/m_axi_fb_ERROR]
	connect_bd_net -net txn_done_00 [get_bd_ports m_axi_fb_TXN_DONE] [get_bd_pins VGA_0/m_axi_fb_TXN_DONE]
	...
	set offset_file "${bd_path}/VGA_v1_0_tb_include.vh"
	set fp [open $offset_file "w"]
	puts $fp "`ifndef VGA_v1_0_tb_include_vh_"
	puts $fp "`define VGA_v1_0_tb_include_vh_\n"
	...
	puts $fp "`define s_axi_config_SLAVE_ADDRESS ${offset_hex}"

	puts $fp "`endif"
	close $fp
}

set ip_path [file dirname [file normalize [get_property XML_FILE_NAME [ipx::get_cores xilinx.com:user:VGA:1.0]]]]
set test_bench_file ${ip_path}/example_designs/bfm_design/VGA_v1_0_tb.v
...
for { set i 1 } { 1 } { incr i } {
	set design_name "VGA_v1_0_bfm_${i}"
...
remove_files -quiet -fileset sim_1 VGA_v1_0_tb_include.vh
import_files -fileset sim_1 -norecurse -force $interface_address_vh_file
set_property top VGA_v1_0_tb [get_filesets sim_1]
